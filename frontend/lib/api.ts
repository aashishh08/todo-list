...
const API_BASE_URL = `${process.env.NEXT_PUBLIC_BACKEND_ENDPOINT}/api`;
...
export const fetchUserTodos = async (
  ...
  console.log(
    `Getting todos: ${API_BASE_URL}/users/${userId}/todos${queryString}`
  );
  const response = await fetch(
    `${API_BASE_URL}/users/${userId}/todos${queryString}`
  );
  if (!response.ok) {
...
export const fetchTodo = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/todos/${todoId}?user=${userId}`
  );
  if (!response.ok) {
    if (response.status === 404) {
...
export const addNoteToTodo = async (
  todoId: string,
  userId: string,
  content: string
): Promise<Note> => {
  const response = await fetch(
    `${API_BASE_URL}/todos/${todoId}/notes?user=${userId}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const exportTodos = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/todos/export?user=${userId}&format=${format}`
  );
